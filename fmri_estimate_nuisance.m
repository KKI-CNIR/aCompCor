function fmri_estimate_nuisance(func_files,tr,rp_file,wm_file,csf_file,brain_mask_file,options,nuisance_file)
%Function to estimate nuisance time courses from the data and use PCA to
...
%                   realignment and nuisance extraction/removal
%                options.diff_motion_params=1; %Use motion params
%                differential? 1=Yes
%                options.square_motion_params=0; %Use square of motion
%                params? 0=No
%                options.global_signal=0; %Use global signal? 0=No
...
%MB Nebel Dec 6, 2012 - fixed error in the way covariance matrices and principal component variances are calculated; limited csf_mask to voxels within ALVIN ventricle mask regardless of mask type chosen
...
if(isfield(options,'wm_signal') && options.wm_signal~=0),%Make mask for either mean or PCA
    ...
    %exclude voxels in probability map with values <= .99
    wm_mask = (wm_mask/max(wm_mask(:)))>0.99;
    %erode the white matter mask
    wm_mask = double(imerode(logical(wm_mask),ones(3,3,3)));
    %if eroded WM mask has fewer than 5 voxels, force ignore options.wm_signal
    if(nnz(wm_mask)<5), 
        options.wm_signal=0;
...
if(isfield(options,'csf_signal') && options.csf_signal~=0), %Make mask for either mean or PCA
    ...
    %If CSF mask is empty force ignore options.csf_signal
    if(nnz(csf_mask) < 5), 
...
if((isfield(options,'wm_signal') && options.wm_signal ~=0) || ...
    ...
    %% initialize variables for white matter, CSF & global signal timeseries
    Y=spm_read_vols(V);
    ...
    %% extract timeseries for voxels in white matter & CSF masks; calculate global signal
    for i=1:size(Y,4),
        tY=Y(:,:,:,i);
        tY(isnan(tY))=0;
    ...
    if(isfield(options,'global_signal') && options.global_signal==1),
        ...
        % add differential of global signal if requested
